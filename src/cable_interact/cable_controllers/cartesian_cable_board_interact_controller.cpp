...
void persist_selected_gripper_frame_asset(const std::string& cable_id,
  ...
  append_json_vector(fields, "selected_gripper_direction", x_axis, true);
...
GraspPlan load_grasp_plan_from_file(const std::string& cable_id) {
  ...
  std::vector<Eigen::Vector3d> gripper_directions;
  if (file_text.find("\"gripper_directions\"") != std::string::npos) {
    gripper_directions = extract_vector3_list(file_text, "gripper_directions");
  } else {
    ...
    gripper_directions.emplace_back(gripper_direction_a[0], gripper_direction_a[1],
                                    gripper_direction_a[2]);
    gripper_directions.emplace_back(gripper_direction_b[0], gripper_direction_b[1],
                                    gripper_direction_b[2]);
  }
...
class CartesianCableBoardInteractController : public controller_interface::ControllerInterface {
 public:
  ...
  CallbackReturn on_configure(const rclcpp_lifecycle::State&) override {
    ...
    gripper_directions_ = {Eigen::Vector3d::UnitX(), -Eigen::Vector3d::UnitX()};
 ...
 private:
  ...
  static Eigen::Quaterniond build_orientation_from_xz_axes(const Eigen::Vector3d& desired_x_axis,
                                                           const Eigen::Vector3d& desired_z_axis) {
    Eigen::Vector3d z_axis = desired_z_axis.normalized();
    Eigen::Vector3d x_axis = project_onto_plane(desired_x_axis, z_axis);
    if (x_axis.norm() < 1e-6) {
      x_axis = Eigen::Vector3d::UnitX();
      x_axis = project_onto_plane(x_axis, z_axis);
    }
    x_axis.normalize();
  ...
  SelectedGripperDirectionDecision choose_gripper_direction_for_target() const {
    ...
    auto evaluate_candidate = [&](const Eigen::Vector3d& direction,
      ...
      const double alignment_score = hover_x_axis.dot(normalized_direction);
      const double delta_joint7 =
          signed_angle_about_axis(hover_x_axis, normalized_direction, kHoverTargetZAxis);
    ...
    const auto selected_candidate = std::max_element(
        candidates.begin(), candidates.end(), [](const CandidateEvaluation& left,
                                                 const CandidateEvaluation& right) {
          if (left.within_limits != right.within_limits) {
            return !left.within_limits && right.within_limits;
          }
          if (left.limit_margin != right.limit_margin) {
            return left.limit_margin < right.limit_margin;
          }
    ...
    if (!selected_candidate->within_limits) {
      RCLCPP_WARN(
          get_node()->get_logger(),
          "No gripper direction keeps estimated joint7 within limits [%.4f, %.4f]. "
          "Choosing candidate %s with largest remaining margin %.4f rad.",
          joint7_limits_.lower, joint7_limits_.upper, selected_candidate->label.c_str(),
  ...
  Eigen::Quaterniond compute_target_orientation(const Eigen::Quaterniond& reference_orientation) const {
    ...
    return build_orientation_from_xz_axes(selected_gripper_direction_, kHoverTargetZAxis);
  }
    ...
    for (const auto& parameter : parameters) {
      if (parameter.get_name() == "target_cable_id") {
        try {
          ...
          requested_target_cable_id = cable_id;
          target_changed = true;
        } catch (const std::exception& exception) {
  ...
  void start_cable_interaction_phase() {
    ...
    RCLCPP_INFO(get_node()->get_logger(),
                "Gripper closed. Starting board cable perturbation recording motion: +/- %.3f m "
                "along target tcp.x then target tcp.y.",
  ...
  void update_cable_interaction_phase(const Eigen::Vector3d& current_position) {
    ...
    if (!segment_active) {
      ...
      motion_phase_ = MotionPhase::kHold;
      RCLCPP_INFO(get_node()->get_logger(),
                  "Board cable perturbation motion finished. Holding the target pose.");
      return;
    }
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianCableBoardInteractController,
