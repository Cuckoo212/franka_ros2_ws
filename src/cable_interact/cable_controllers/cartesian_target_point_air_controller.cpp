...
std::filesystem::path build_viewer_selection_asset_path(const std::string& cable_id) {
  const std::string normalized_cable_id = normalize_cable_id(cable_id);
  const std::filesystem::path cable_dir =
      std::filesystem::path(kGraspPointRoot) / ("cable_" + normalized_cable_id);
...
class CartesianTargetPointAirController : public controller_interface::ControllerInterface {
 public:
...
PLUGINLIB_EXPORT_CLASS(franka_example_controllers::CartesianTargetPointAirController,
